...
def draw_quote(base, progress=1.0):
    font = f(HIM_P, 48)
    lines = ['"친구가 좋다고 해서..."', '"패키지로 그냥 가입했는데"']
...
def draw_checklist(base, progress_per_item):
    items = [
        ("보장 범위 (암/골절/화상/식중독)", "", False),
        ("면책기간 체크", "", False),
        ("갱신형 vs 비갱신형", "", True),
        ("특약 꼼꼼히 살피기", "", False),
        ("보험사 안정성 (RBC 150%↑)", "", True),
    ]
    font = f(NANUM_P, 52)
    ...
    disc = "* 가입 조건은 보험사·상품별로 달라질 수 있어요"
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_05_preview.png')
