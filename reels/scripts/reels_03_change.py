"""
릴스 03: 보험 바꿔도 될까 - 보험 리모델링 망설임 해소
...
def draw_title(base, progress=1.0):
    font = f(GAMJA_P, 86)
    lines = ["바꾸고 싶은데", "왜 망설이게 될까요?"]
    _draw_text_lines(base, lines, font, 220, INK, 104, progress, stagger=0.45)

def draw_body(base, progress=1.0, hl_progress=1.0):
    font = f(NANUM_P, 62)
    lines = ["지금 그 불안,", "근거 있는지 확인해봐요"]
    y_start = 470
    if hl_progress > 0:
...
def draw_quote(base, progress=1.0):
    font = f(HIM_P, 48)
    lines = ['"새로 가입하면 거절될까봐..."', '"보험료 오를까봐, 면책기간이..."']
...
def draw_checklist(base, progress_per_item):
    ...
    disc = "* 가입 상황·이력에 따라 결과가 달라질 수 있어요"
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_03_preview.png')
