...
def draw_title(base, progress=1.0):
    font = f(GAMJA_P, 94)
    lines = ["당신의 집은,", "안전한가요?"]
    _draw_text_lines(base, lines, font, 220, INK, 110, progress, stagger=0.45)

def draw_body(base, progress=1.0, hl_progress=1.0):
    font = f(NANUM_P, 62)
    lines = ["하루 평균", "103건 발생해요"]
    y_start = 480
    if hl_progress > 0:
...
def draw_checklist(base, progress_per_item):
    items = [
        ("10년간 화재 약 40만 건", "", False),
        ("2024 주거 화재 사망 174명", "", False),
        ("주거 재산피해 712억원", "", True),
        ("원인 1·2위 부주의/전기적 요인", "", False),
        ("실화책임법 - 이웃 피해도 배상", "", True),
    ]
    font = f(NANUM_P, 52)
    ...
    disc = "* 출처: 소방청 2024년도 화재통계연감, 2025.06"
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_10_preview.png')
