...
def draw_title(base, progress=1.0):
    font = f(GAMJA_P, 90)
    lines = ["사망원인", "1·2·4위 아세요?"]
    _draw_text_lines(base, lines, font, 220, INK, 108, progress, stagger=0.45)
...
def draw_checklist(base, progress_per_item):
    items = [
        ("암 1인 평균 진료비 약 520만원", "", False),
        ("뇌졸중 입원 진료비 약 1,593만원", "", True),
        ("심장질환 진료비 약 1,384만원", "", False),
        ("비급여(표적항암제 등) 전액 본인", "", True),
        ("65세 진료비, 평균의 2.4배", "", False),
    ]
    font = f(NANUM_P, 52)
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_09_preview.png')
