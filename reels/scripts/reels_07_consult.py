...
def draw_quote(base, progress=1.0):
    font = f(HIM_P, 48)
    lines = ['"한 상품만 자꾸 권하던데..."', '"결정 재촉하니까 불안해요"']
...
def draw_checklist(base, progress_per_item):
    items = [
        ("복수 회사 상품 실제 비교", "", True),
        ("면책기간·보장 제외 명확 설명", "", False),
        ("내 상황(나이·건강) 맞춤 분석", "", False),
        ("승환계약 불이익 먼저 고지", "", True),
        ("금소법 19조 적합성·적정성 준수", "", False),
    ]
    font = f(NANUM_P, 52)
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_07_preview.png')
