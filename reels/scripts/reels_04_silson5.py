...
def draw_quote(base, progress=1.0):
    font = f(HIM_P, 48)
    lines = ['"매년 보험료가 오르네..."', '"5세대로 가도 괜찮을까?"']
...
def draw_checklist(base, progress_per_item):
    items = [
        ("비급여 중증/비중증 분리", "", False),
        ("중증만 가입 시 약 50% 인하", "", True),
        ("비중증은 할인·할증 적용", "", False),
        ("4세대 대비 30~50% 인하 기대", "", True),
        ("5년 재가입 주기 / 25년말 출시", "", False),
    ]
    font = f(NANUM_P, 52)
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_04_preview.png')
