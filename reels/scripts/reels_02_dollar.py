...
def draw_title(base, progress=1.0):
    font = f(GAMJA_P, 96)
    lines = ["원화만 믿다가", "괜찮으신가요?"]
    _draw_text_lines(base, lines, font, 220, INK, 112, progress, stagger=0.45)

def draw_body(base, progress=1.0, hl_progress=1.0):
    font = f(NANUM_P, 64)
    lines = ["달러로 준비하는", "평생 연금 설계"]
    y_start = 480
    if hl_progress > 0:
...
def draw_quote(base, progress=1.0):
    font = f(HIM_P, 50)
    lines = ['"환율 오를 때마다 불안한데..."', '"달러로 연금 받을 순 없나?" 싶었다면']
...
def draw_checklist(base, progress_per_item):
    ...
    disc = "* 환율에 따라 보험금·환급금이 달라질 수 있어요"
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_02_preview.png')
