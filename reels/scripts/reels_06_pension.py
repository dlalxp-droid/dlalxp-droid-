...
def draw_title(base, progress=1.0):
    font = f(GAMJA_P, 84)
    lines = ["은퇴 후 매달", "336만원, 아셨나요?"]
    _draw_text_lines(base, lines, font, 220, INK, 102, progress, stagger=0.45)

def draw_body(base, progress=1.0, hl_progress=1.0):
    font = f(NANUM_P, 62)
    lines = ["국민연금만으론", "한참 부족해요"]
    y_start = 470
    if hl_progress > 0:
...
def draw_quote(base, progress=1.0):
    font = f(HIM_P, 48)
    lines = ['"국민연금 있으니 어떻게든..."', '"아직 멀었으니까 천천히"']
    _draw_text_lines(base, lines, font, 680, PENCIL, 64, progress, stagger=0.35)
...
def draw_checklist(base, progress_per_item):
    items = [
        ("부부 적정 생활비 월 336만원", "", False),
        ("국민연금 평균 월 67만원", "", False),
        ("매달 173~269만원 부족", "", True),
        ("65세 기대여명 20.7년", "", False),
        ("연금은 복리 - 일찍 시작 유리", "", True),
    ]
    font = f(NANUM_P, 52)
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_06_preview.png')
