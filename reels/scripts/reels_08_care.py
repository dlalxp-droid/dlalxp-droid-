...
def draw_title(base, progress=1.0):
    font = f(GAMJA_P, 88)
    lines = ["내가 아프면,", "누가 돌봐줄까요?"]
    _draw_text_lines(base, lines, font, 220, INK, 106, progress, stagger=0.45)
...
def draw_quote(base, progress=1.0):
    font = f(HIM_P, 48)
    lines = ['"아직 젊으니까 괜찮아..."', '"가족 있는데 뭘 걱정해"']
...
def draw_checklist(base, progress_per_item):
    items = [
        ("월평균 간병비 약 370만원", "", True),
        ("하루 간병비만 약 11만원", "", False),
        ("1인가구 36.1% (804만 가구)", "", False),
        ("73%가 간병 경제적 준비 안 됨", "", True),
        ("간병 보험 가입은 29%뿐", "", False),
    ]
    font = f(NANUM_P, 52)
...
def render_static():
    ...
    path = os.path.join(out_dir, 'static_08_preview.png')
